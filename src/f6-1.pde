...
void createFont() {
  ...
  for (char c : Fontastic.alphabet) {
    ...
    for (int tt = 0; tt < mpoints.length; tt++) {
         ...
         if (tt % 5 ==0) { //random(100) < 30) {
           PVector[] newpoi = new PVector[4];
           newpoi[0] = (new PVector(mpoints[tt].x, mpoints[tt].y));
           newpoi[1] = (new PVector(mpoints[tt].x, mpoints[tt].y + 200));
           newpoi[2] = (new PVector(mpoints[tt].x + 10, mpoints[tt].y));
           newpoi[3] = (new PVector(mpoints[tt].x + 10, mpoints[tt].y + 200));
           f.getGlyph(c).addContour(newpoi);
         }
      
        //if (tt % 4 ==0) {
//            mpoints[tt].y = mpoints[tt].y + 10 * cos(tt * 6.28 * 10 / mpoints.length);  
//            mpoints[tt].x = mpoints[tt].x + 10 * sin(tt * 6.28 * 5 / mpoints.length); 
        //}
