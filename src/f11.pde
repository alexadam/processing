...
void createFont() {
  ...
  for (char c : Fontastic.alphabet) {
    ...
    for (int ii=0; ii<pnts.length-1; ii++) {
      if (ii % 3 == 0) {
        RPoint p = pnts[ii];
        points = (FPoint[]) append(points, new FPoint(new PVector(p.x * 5, -p.y * 5), new PVector(p.x * 5 * 3.14, -p.y * 5 * 3.14), new PVector(p.x * 5 + 2, -p.y * 5 + random(1, 2))));
      }
    }
